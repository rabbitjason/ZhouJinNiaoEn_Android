...
public class UserMananger {
	...
	/**更新的内容*/
	public static void dyios(Context context,BaseCallListener pListener){


//		http://139.224.220.166/Goldpecker/App/User/dyios
		//String url = Config.hostString +"App/User/dyios";
        String url = "http://139.224.220.166/Goldpecker/App/User/dyios";

		BaseHttpManager.get(context, url, null, OtherResponse.class, pListener);
	}


    /** 消息列表 */
    public static void getMsg(Context context, int page_number,
                              BaseCallListener pListener) {
        // "1d29dea7b275efa30908700a1d3a6cf8"
        String url = Config.hostString
                + "/App/User/getMsg?uid="
                + PreferenceManager.getUserId()
                + "&page_number="
                ...
                String.valueOf(page_number)
                        + String.valueOf(Config.listCount)
                        + PreferenceManager.getUserId()
                        + "aVQ@iUH0CNnpQT0Q").toLowerCase();

        BaseHttpManager.get(context, url, null, MegResponse.class, pListener);
    }

    public static void read(Context context, String id,
                            BaseCallListener pListener) {

        // http://139.224.220.166/Goldpecker/App/User/readMsg?id=ID&uid=UID
        ...
        String url = Config.hostString
                ...
                + "&sign="
                + Utils.MD5(
                id + PreferenceManager.getUserId() + "aVQ@iUH0CNnpQT0Q")
                .toLowerCase();

        BaseHttpManager.get(context, url, null, BaseResponse.class, pListener);
    }
}
